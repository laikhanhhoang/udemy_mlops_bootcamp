...
| 🏛️ **`models:/`** | `models:/<name>/<version_or_stage>` | Truy cập tài nguyên đã được đăng ký trong Model Registry. |
| 📂 **`file:/`** | `file:/path/to/directory` | Truy cập tài nguyên trực tiếp từ hệ thống tệp cục bộ (Local Filesystem). |
| ☁️ **`s3:/`** | `s3://<bucket>/<path>` | Truy cập tài nguyên được lưu trữ trên Amazon S3 (hoặc các object storage tương đương). |